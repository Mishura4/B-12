#ifndef SHION_REGISTRY_H_
#define SHION_REGISTRY_H_
...
namespace shion
{
  ...
  namespace _
  {
    ...
    {
      using type = std::conditional_t<(sizeof(T) > (sizeof(void *) * 4)), shion::variable_ref<const T>, T>;
    };
  ...
  struct registry : private Fields...
  {
    ...
    template <typename T = shion::type_list<Fields...>>
    requires (sizeof...(Fields) > 0 && (std::is_default_constructible_v<Fields> && ...))
    constexpr registry() :
      Fields{}...
    {
      static_assert(!shion::has_duplicate_values<true, Fields::key...>, "cannot have duplicate keys");
    }
  ...
  namespace _
  {
    ...
    struct dynamic_registry_value_type_s<shion::type_list<Args...>>
    {
      using type = std::variant<dynamic_registry_storage<Args>...>;
      using ref_type = shion::variable_ref<Args...>;
    };
    ...
    template <typename... Args>
    struct dynamic_key_type_s<shion::type_list<Args...>>
    {
      using type = typename shion::type_list<typename field_runtime_key_type_s<Args>::type...>::without_duplicate_types;
    };
    ...
    struct registry_helper<T, Args...>
    {
      using type = typename shion::type_list<T>::template cat<typename registry_helper<Args...>::type>;
    };
    ...
    template <typename... Args>
    struct registry_list_helper<shion::type_list<Args...>>
    {
      using field_list = shion::type_list<Args...>;
      using key_list = shion::value_list<registry_key_get<Args>...>;
      using key_type_list = typename key_list::type_list;
      using value_type_list = typename shion::type_list<typename Args::value_type...>::without_duplicate_types;
    ...
    {
      using return_type = shion::variable_ref<typename Args::type...>;
      ...
      using type = typename shion::variable_ref<typename Args::type...>;
    };
  };
  ...
  dynamic_registry(Ts...) -> dynamic_registry<std::map<typename _::registry_list_helper<typename _::registry_helper<Ts...>::type>::dynamic_key_type, typename _::registry_list_helper<typename _::registry_helper<Ts...>::type>::value_type>, Ts...>;
